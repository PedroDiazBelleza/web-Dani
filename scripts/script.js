...
//Fecha final
const fechaFinal = new Date('Mar 8, 2024 23:50:15').getTime();
//1709279400000
...
document.querySelectorAll(".carousel").forEach((carousel) => {
    ...
    // Función para mostrar el siguiente elemento del carousel
    function showNext() {
        items[currentIndex].classList.remove("carousel_item--selected");
        buttons[currentIndex].classList.remove("carousel_button--selected");

        currentIndex = (currentIndex + 1) % items.length;

        items[currentIndex].classList.add("carousel_item--selected");
        buttons[currentIndex].classList.add("carousel_button--selected");
    }

    // Configuración del intervalo para el desplazamiento automático
    const autoScrollInterval = setInterval(showNext, 3000); // Cambia la duración en milisegundos según tus preferencias
});*/